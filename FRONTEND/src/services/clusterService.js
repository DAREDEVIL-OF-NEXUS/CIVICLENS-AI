...
export function sortComplaintsForQueue(complaints = []) {
...
export function buildActivePriorityQueue(complaints = []) {
  return sortComplaintsForQueue(
    complaints.filter(
      (complaint) =>
        complaint?.status !== "RESOLVED" && complaint?.duplicate_of === null
    )
  );
}

export function buildDuplicateQueue(complaints = []) {
  return sortComplaintsForQueue(
    complaints.filter(
      (complaint) =>
        complaint?.status !== "RESOLVED" && complaint?.duplicate_of !== null
...
export function buildResolvedComplaints(complaints = []) {
  return sortComplaintsForQueue(
    complaints.filter((complaint) => complaint?.status === "RESOLVED")
  );
}

export function applyUserComplaintPreference(
  complaints = [],
  currentUsername = "",
  ...
  if (showOnlyMyComplaints) {
    return complaints.filter(
      (complaint) => String(complaint?.submitted_by || "").trim() === cleanUsername
    );
  }
